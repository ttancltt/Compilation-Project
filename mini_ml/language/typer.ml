...
let rec type_expr (counter : Counter.t) (env : type_lang Util.Environment.t)
  ...
  | Cst_func (builtin, ann) ->
      let typ = instantiate counter (type_of_built_in builtin) in
      annotate expr typ, []
  | Nil ann ->
  ...
  | Var (name, ann) ->
      ...
      | Some typ ->
          let typ = instantiate counter typ in
          annotate expr typ, []
      | None ->
          raise
            (Typing_error
  ...
  | Let (recursive, name, e1, e2, ann) ->
      (* floor is the current counter value before typing the let binding *)
      let floor = Typer_util.Counter.get_fresh counter - 1 in
      if recursive then
        let placeholder = fresh_type counter in
        Util.Environment.add env name placeholder;
        let t1, c1 = type_expr counter env e1 in
        let c1 = (placeholder, t1) :: c1 in
        let subst = solve_constraints c1 in
        Type_system.type_substitution_in_expr e1 subst;
        let t1 = Type_system.apply_subst_in_type subst t1 in
        let typ = generalize_type_expr floor e1 in
